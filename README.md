...
进度18引入maven管理实体类和数据库接口

差不多得啦，自带的Maven不能用，得重下一个
然后镜像又要重新设置，包也要重新导
好不容易导完包，命令也有问题，这是官网的命令耶
生成了一个以后，没啥问题，对着函数名，
查一下用法就可以对着函数名重新映射了
但是为什么另一个表导入又可以报错，尼玛的



差了半天，原来是XML文件生成错误，多成成了一个插件查询
导致SQL语句不能一一对应，之后把重复的删掉，项目可以编译了
...
首页显示逻辑——统计所有的question数量
当且仅当没有数据时，表为空

个人界面显示逻辑——根据创建者ID来查询question
个人数据可能为空，但是总数据不为空

设置一个默认的createor作为参数？如果值为默认值，则认为是首页查询，否则
将createor=-1作为主页查询的标识，当useid为这个值的时候，两句sql查询不一致
...
进度19编写异处理，统合发生异常时的界面

第一次写这个东西- -，感觉好复杂
界面的话，用bootstrap写一个简单的显示

唔，查了一下
异常处理需要写五个东西
分别是
接口类：用于存放获取异常信息的接口函数
枚举类：用于实现接口类的函数，并且枚举出不同异常的值
以下为请求码对应的情况

200 成功

400 服务器未能理解请求（地址写错了呗）
401 被请求界面需要用户名和密码（这个不一定用，一般跳转到登录界面）
404 服务器无法找到被请求的界面（没有对应资源）

500 请求未完成，服务器遇到不可预知的情况（自己的BUG，呜呜呜）
...
额，因为同样是404，但是在不同的界面访问，我希望出现的提示不一样
目前默认的提示是资源未找到
而在问题界面抛出的异常是，该问题被删除或者不存在

进度19改 完善了各种异常的机制，添加了常见的几种异常
但是目前来说，会经常触发的只有404和500了
其他的要通过手写BUG才能验证了

进度20添加了通过点击页码增加阅读数的功能
...
进度22编写回复界面
将输入的文本框弄好了，也能够回复并且正确的写入了
如果出错了，还会返回错误信息的弹窗。
JS好麻烦，如果可以不写，真的不想碰这个玩意= =
接下来考虑怎么把评论列表展示出来就行了

进度23实现弹窗登录功能
又是必须用JS来实现界面之间的转换和跳转
好麻烦啊，而且前端的调试非常困难
而且刷新和数据内容的同步也特别麻烦
不过总算在未登录时，通过弹窗跳转至登录页面进行自动登录

进度24通过之前的页码进行复用，重写了评论分页
还好之前对分页的函数进行了统合接口，
现在评论分页完美的对应上了各种接口
就是还有点小问题，我暂时没有做url的正确拼接
所以现在的分页显示是有bug的，这个将在后续的时候进行修复
